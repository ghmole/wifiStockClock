...
def get_wifi_conf(request):
    ...
    if len(ssid) == 0 or len(cityname) == 0 or len(citypy) == 0: #wifi名称或者城市名称为空。
        return None
                    ...
                    if ret != None: #获取信息成功
                        ...
                        if city_judge(ret[2]): #有该城市
                            ...
                            if ret_ip is not None:
                                ...
                                city_info = {'CITY':'','CITY_PY':''}
                                city_info['CITY'] = unquote(ret[2]).decode("gbk")
                                city_info['CITY_PY'] = ret[3]
                                ...
                                while True:
                                    text = f.readline()
                                    if city_info['CITY'] in text:
                                        if city_info['CITY'] == re.match(r'"(.+?)"',text).group(1): # 城市名称完全一样
                                            city_code = re.match(r'"(.+?)"',text.split(': ')[1]).group(1) # 获取城市编码
                                            break
                                    elif '}' in text: #结束，没这个城市。
                                        print('No City Name!')
                                ...
                                f = open('/data/config/wifi.cfg', 'w') #以写的方式打开一个文件，没有该文件就自动新建
                                f.write(json.dumps(wifi_info)) #写入数据
                                f.close() #每次操作完记得关闭文件
                                
                                f = open('/data/config/city.cfg', 'w') #以写的方式打开一个文件，没有该文件就自动新建
                                f.write(json.dumps(city_info)) #写入数据
                                f.close() #每次操作完记得关闭文件
